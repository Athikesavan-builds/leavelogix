...
// Database config - change these to your DB
$db_host = 'localhost';
$db_user = 'root';
$db_pass = '';
$db_name = 'leavelogix';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update DB
    $stmt = $conn->prepare("UPDATE admins SET fullName=?, email=?, department=?, designation=?, avatar=? WHERE id=?");
    $stmt->bind_param('sssssi', $fullName, $email, $department, $designation, $avatarFileName, $admin_id);
    $stmt->execute();
    $profile = ['fullName'=>$fullName,'email'=>$email,'department'=>$department,'designation'=>$designation,'avatar'=>$avatarFileName];
...
<body>
  ...
  <main class="wrap" role="main">
    ...
    <aside class="left-card" aria-label="Profile summary">
      ...
      <div class="user-sub" id="displayDept">Department</div>
      <div class="user-sub" id="displayRole">Designation</div>
      ...
      <form id="profileForm">
        <div class="form-grid">
          ...
          <div>
            ...
            <input id="designation" type="text" placeholder="Software Engineer" />
          </div>
        </div>
